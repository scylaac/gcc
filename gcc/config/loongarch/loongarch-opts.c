  ...
  int* isa_ext_flags, int* abi_int, int* abi_float)
{
  ...
  */
  ...
  /* For native compilers, gather local CPU information 
    ...
    #ifdef __loongarch__
     /* Read cpucfg in native compilers. */
     cache_cpucfg();
  ...
  if (LARCH_OPT_ABSENT(*cpu_tune)
   && LARCH_OPT_ABSENT(*cpu_arch))
    {
      // Both not seen, using configure-time defaults.
      *cpu_arch = DEFAULT_CPU_ARCH;
      *cpu_tune = DEFAULT_CPU_TUNE;
    }
  else if (LARCH_OPT_ABSENT(*cpu_tune))
    {
      // Try inferring -mtune from -march.
      *cpu_tune = *cpu_arch;
    }
  else if (LARCH_OPT_ABSENT(*cpu_arch))
    {
      /* Do not infer -march from -mtune, 
         it should be set to configure-time default. */
      *cpu_arch = DEFAULT_CPU_ARCH;
    }
  ...
  *isa_ext_flags
    = loongarch_cpu_default_config[*cpu_arch].isa_ext_flags;
  ...
  if (LARCH_OPT_ABSENT(*isa_float))
    {
      ...
      // If "-march" wasn't present, set *isa_float to configure-time default.
